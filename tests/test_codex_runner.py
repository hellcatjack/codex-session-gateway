...
def test_codex_runner_build_args_with_auto_resume(monkeypatch, tmp_path):
    ...
    # newer subagent session should be ignored
    write_session(
        "sess-subagent",
        "/proj",
    ...
    # newest root session should be selected
    write_session("sess-new", "/proj", "2026-01-03T00:00:00Z", "cli")
...
async def test_codex_runner_run_resolves_auto_for_jsonl_tailer(monkeypatch, tmp_path):
    ...
    append_task = asyncio.create_task(append_event())
...
async def test_codex_runner_does_not_duplicate_final_message_when_already_streamed(tmp_path):
    ...
    assert return_code == 0
    assert finals == ["line1\nline2"]
    assert "line1" in outputs
    assert "line2" in outputs
    # The final message should not be emitted again as a combined block if it was
    # already streamed line-by-line.
    assert "line1\nline2" not in outputs
...
async def test_codex_runner_no_progress_heartbeat(tmp_path):
    ...
    await runner.run("hello", on_output, on_status)
    assert not any("进度：" in text for text in outputs)
